...
module.exports = cds.service.impl( async function(){
    ...
    const { EmpployeeDetails, PODetails } = this.entities;
    const { uuid } = cds.utils;

    // Step - 2 : Develop a generic handler for validation of Employees Set
    this.before('UPDATE', EmpployeeDetails, (request, response) =>{
...
//        response = await transaction.run( SELECT.from(PODetails).where(ID));
    ...
    try {
        ...
        // Step - 2 : Read the highest order using PO Details
...
this.on('createEmployee', async (request, resopnse) => {
     ...
     this.on( 'getProducts', async(request, response) => {
        ...
        try {
            ...
            finalResponse = await cds.run(SELECT.from('POCAP_DB_MASTER_PRODUCT').columns('PRODUCT_ID', 'TYPE_CODE', 'CATEGORY', 'DESCRIPTION', 'PRICE', 'CURRENCY_CODE'
        ).where({ CURRENCY_CODE: { '=': tempCode } }));
